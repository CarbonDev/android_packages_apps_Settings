...
public class StylusGestures extends SettingsPreferenceFragment implements
    ...
    private void setPrefValue(ListPreference pref, String packageName) {
        ...
        if (text != null) {
            ...
            if (appName != null) {
                ...
                // pref.setValue(String.valueOf(KEY_NO_ACTION));
    ...
    private void addApplicationEntries(ListPreference pref, String packageName) {
        ...
        List<String> existingEntry = Arrays.asList(actionNames);
        List<String> existingEntryValue = Arrays.asList(actionValues);
        Map<String, String> prefMap = new TreeMap<String, String>();
        int size = existingEntry.size();
        for (int i = 0; i < size; i++) {
            prefMap.put(existingEntry.get(i), existingEntryValue.get(i));
        }
        Intent intent = new Intent(Intent.ACTION_MAIN, null);
        intent.addCategory(Intent.CATEGORY_LAUNCHER);
        List<ResolveInfo> list = pm.queryIntentActivities(intent,
                PackageManager.PERMISSION_GRANTED);
        List<String> entryListValue = new ArrayList<String>();
        List<String> entryList = new ArrayList<String>();
        // if (String.valueOf(KEY_NO_ACTION).equalsIgnoreCase(packageName)) {
        // flag = true;
        // }
